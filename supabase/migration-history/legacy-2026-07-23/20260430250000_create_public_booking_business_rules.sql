-- SINGLE SOURCE OF TRUTH for booking RPC
-- Do not redefine in later migrations
--
-- Harden create_public_booking: deterministic P0001 validation, UTC opening_hours parity (TODO: salons.timezone), overlap pre-check, JSON error mapping.
...
declare
  ...
  v_digits text;
begin
  raise notice 'create_public_booking v2.2';
...
comment on function public.create_public_booking(
  uuid, uuid, uuid, text, text, timestamptz, timestamptz, text, integer, text, uuid, integer
) is 'SINGLE SOURCE (v2.2): atomic public booking — P0001 validation, opening_hours + closed dates (UTC), advisory lock, overlap, JSON outcomes.';
