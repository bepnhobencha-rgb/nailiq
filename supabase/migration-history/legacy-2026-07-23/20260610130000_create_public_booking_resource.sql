-- Resource layer (Phase 1, module 3): thread an OPTIONAL p_resource_id through
-- create_public_booking. When NULL (every existing caller + resources-off salon)
-- the function behaves EXACTLY as v2.5 — all resource logic is null-guarded.
-- When set (resource-mode salons), it validates the resource belongs to the
-- salon, takes a per-resource advisory lock, pre-checks resource overlap, and
-- stores resource_id. The bookings_resource_no_overlap GIST is the race backstop.
...
declare
  ...
  v_email text;
begin
  raise notice 'create_public_booking v2.6 (with resource_id)';
  ...
  -- Resource (optional): must belong to the salon and be live.
  ...
  -- Resource overlap pre-check (separate advisory lock key) when in resource-mode.
  if p_resource_id is not null then
    perform pg_advisory_xact_lock(
      hashtext(p_salon_id::text || chr(254) || p_resource_id::text)::bigint
    );
    if exists (
      select 1
      from public.bookings b
