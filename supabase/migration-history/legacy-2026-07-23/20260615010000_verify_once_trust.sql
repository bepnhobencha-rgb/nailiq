-- "Verify once, trust" — add a short-circuit so a phone that has ALREADY passed
-- OTP isn't re-challenged on every future booking (modern low-friction UX).
-- Safety: only trusts a phone that is clean (no no-show on record) AND verified
-- within the last 12 months; new phones, phones with a no-show, and dormant
-- phones still verify. Sits alongside the existing VIP / trusted-returning
-- short-circuits, so it applies regardless of booking_verification_mode (same
-- precedent as trusted_returning bypassing always_otp). The card-on-file / deposit
-- gate is independent and still protects revenue for new / high-risk customers.
...
BEGIN
  ...
  -- Look up client profile by phone (no salon_id on client_profiles)
  SELECT is_vip,
  ...
  -- "Verify once, trust": a phone that already passed OTP is trusted on future
  -- bookings — UNLESS it has a no-show (risk signal) or the verification is stale
  -- (>12 months). New/no-show/dormant phones fall through and still verify.
