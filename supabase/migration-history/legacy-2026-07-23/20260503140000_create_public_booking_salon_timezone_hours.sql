-- create_public_booking v2.4: interpret opening_hours + booking_closed_dates in salons.timezone
-- (IANA). Fixes false outside_hours for evenings/weekends when UTC calendar wall-clock diverges
-- from the salon's local day (e.g. Sun 4pm PT stored as 23:00 UTC vs hours 10:00–17:00 local).
...
declare
  ...
  v_digits text;
begin
  raise notice 'create_public_booking v2.4';
  ...
  -- Opening hours + closed dates: salon-local calendar (salons.timezone).
  ...
  -- Whole block must fit inside [open, close] (matches client assertSlotWithinOpeningHours).
...
comment on function public.create_public_booking(
  uuid, uuid, uuid, text, text, timestamptz, timestamptz, text, integer, text, uuid, integer
) is 'PUBLIC BOOKING RPC v2.4 — opening_hours + closed dates in salons.timezone; block end-after-close; desk-aligned overlap + advisory lock; JSON outcomes.';
