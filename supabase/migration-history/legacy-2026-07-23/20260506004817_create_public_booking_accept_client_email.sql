-- B-10 follow-up: thread `p_client_email` through the create_public_booking
-- RPC so the email entered on the booking form actually persists.
--
-- Old signature is dropped and recreated with one extra trailing param so
-- PostgREST routes by name+arg-set without ambiguity.

DROP FUNCTION IF EXISTS public.create_public_booking(
  uuid, uuid, uuid, text, text, timestamptz, timestamptz,
  text, integer, text, uuid, integer
);
...
declare
  ...
  v_email text;
begin
  raise notice 'create_public_booking v2.5 (with client_email)';
