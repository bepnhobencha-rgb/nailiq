-- Restore the production-applied public booking resource projection to source
-- control. Anonymous booking snapshots need the public bed/chair identity, but
-- private resource rows and writes stay behind the existing member-only RLS.
...
COMMENT ON VIEW public.public_booking_resource_catalog IS
  'Read-only public booking projection: active resource identity and kind only, from published resource-enabled salons. No client, occupancy, financial, or private resource metadata.';
