...
-- The edge runtime reaches the ledger only through the narrow functions above.
-- Keep raw rows unreachable even to PostgREST's service_role.
revoke all on table public.sms_delivery_attempts from service_role;

-- Per-salon switches exist only for optional SMS experiences. Required
-- transactional receipts are always on in application policy.
create table if not exists public.salon_sms_template_settings (
  salon_id uuid primary key references public.salons(id) on delete cascade,
  settings jsonb not null default '{}'::jsonb,
  updated_by uuid references auth.users(id) on delete set null,
  updated_at timestamptz not null default clock_timestamp(),
  constraint salon_sms_template_settings_object_check check (
    jsonb_typeof(settings) = 'object'
  )
);

alter table public.salon_sms_template_settings enable row level security;
