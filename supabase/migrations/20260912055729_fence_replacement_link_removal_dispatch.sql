...
BEGIN
 ...
 END IF;
 -- A renewed capability/request is not permission to repeat an unresolved
 -- removal of this same card. The booking/group lock also serializes this
 -- check with dispatches belonging to a different capability.
 IF EXISTS(SELECT 1 FROM public.booking_card_removal_dispatch_bindings prior
   JOIN public.booking_card_management_operations previous ON previous.id=prior.operation_id
   WHERE prior.salon_id=o.salon_id AND prior.booking_id=o.booking_id
     AND prior.card_id=b.noshow_card_id AND prior.customer_id=b.noshow_customer_id
     AND prior.operation_id<>o.id AND previous.status IN ('sending','unknown')
     AND NOT EXISTS(SELECT 1 FROM public.booking_card_removal_recovery_receipts receipt
       WHERE receipt.operation_id=prior.operation_id)) THEN
   RETURN jsonb_build_object('ok',false,'code','removal_dispatch_in_progress'); END IF;
