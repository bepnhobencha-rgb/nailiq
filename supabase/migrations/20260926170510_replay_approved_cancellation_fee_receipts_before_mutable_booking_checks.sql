-- Replay approved cancellation receipts before mutable booking/card checks.
-- Keep review -> booking -> operation lock order and all tenant/approval gates.
CREATE OR REPLACE FUNCTION public.claim_approved_cancellation_fee_payment(p_review_kind text, p_review_id uuid, p_salon_id uuid, p_actor_user_id uuid, p_actor_role text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
  ...
  IF FOUND THEN
    IF v_existing.booking_id IS DISTINCT FROM v_booking_id
       ...
       OR (p_review_kind='late' AND v_existing.operation_occurrence_version IS DISTINCT FROM v_occurrence)
       OR v_existing.material_json#>>'{cancel_preview,review_kind}' IS DISTINCT FROM p_review_kind
       OR v_existing.material_json#>>'{cancel_preview,review_id}' IS DISTINCT FROM p_review_id::text
       OR v_existing.material_json#>>'{cancel_preview,approval_request_id}' IS DISTINCT FROM v_request_id::text
       OR (p_review_kind='late' AND v_late.payment_operation_id IS NOT NULL
           AND v_late.payment_operation_id IS DISTINCT FROM v_existing.id)
       OR (p_review_kind='group' AND v_group.payment_operation_id IS NOT NULL
           AND v_group.payment_operation_id IS DISTINCT FROM v_existing.id) THEN
      RETURN jsonb_build_object('success', false, 'code', 'operation_conflict');
    END IF;
    IF v_existing.status = 'succeeded' THEN
      IF nullif(trim(v_existing.provider_payment_id),'') IS NULL
         OR v_existing.result_json->>'provider_payment_id' IS DISTINCT FROM v_existing.provider_payment_id THEN
        RETURN jsonb_build_object('success',false,'code','payment_replay_receipt_invalid',
          'operation_id',v_existing.id,'status','unknown');
      END IF;
  ...
  -- Only NEW operations inspect current booking/card eligibility. Exact replay
  -- above is bound to the immutable approval and never creates a new charge.
  ...
  -- NEW operations only. Existing outcomes above retain their exact immutable
