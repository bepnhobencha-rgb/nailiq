-- TurnIQ authenticated dashboard visibility boundary.
--
-- The core TurnIQ ledger and commands are service-role only, but every
-- authorized salon member must receive the single bounded boolean that gates
-- TurnIQ UI. The broader management feature_flags JSONB remains private.
--
-- Rollback: restore the previous function definition without
-- turniq_trust_engine_enabled. Do not change salon feature flags or ledger data.
...
COMMENT ON FUNCTION public.load_salon_member_operational_profile(uuid) IS
  'Active-session tenant-member operational salon loader with bounded release booleans, normalized notification/segment settings and no owner PII or provider identifiers.';
