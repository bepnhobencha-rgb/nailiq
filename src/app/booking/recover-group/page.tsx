...
import { clearPendingGroupCreate, parsePendingGroupCreate } from "@/shared/booking/pendingGroupCreate";
...
export default function RecoverGroupPage() {
  ...
  const binding = parsePendingGroupCreate(hash);
  ...
  async function check() {
    ...
    try {
      const response = await fetch("/api/booking/group-create-recovery", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(binding), cache: "no-store", signal: controller.signal });
      ...
      if (response.ok && value.ok === true && value.status === "found" && recovered && recovered.salonId === binding.salonId && recovered.idempotencyKey === binding.idempotencyKey && recovered.pricingFingerprint === binding.pricingFingerprint) {
        try { clearPendingGroupCreate(window.sessionStorage, binding); } catch { /* The fragment still carries authority. */ }
  ...
  return <main style={theme} className="min-h-screen bg-[var(--booking-bg)] text-[var(--booking-text)]">
    <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center gap-6 px-5 py-10">
      <h1 className="text-2xl font-semibold">{vi ? "Kiểm tra lịch nhóm" : "Check group booking"}</h1>
      {status !== "resolved" ? <p className="text-sm leading-relaxed text-[var(--booking-text-muted)]">{vi ? "Kết nối bị gián đoạn nên chúng tôi chưa nhận được kết quả đặt lịch. Lịch nhóm có thể đã được giữ. Vui lòng kiểm tra trước khi đặt thêm." : "The connection was interrupted before we received your booking result. Your group may already be reserved. Please check before making another booking."}</p> : null}
      ...
      {binding && (status === "pending" || status === "expired" || status === "resolved") ? <PendingBookingCreateResolution key={hash} binding={{ ...binding, kind: "group" }} vi={vi} onResolved={() => { clearPendingGroupCreate(window.sessionStorage, binding); setResult({ hash, status: "resolved" }); }} /> : null}
