...
export async function POST(req: NextRequest) {
  let sdp_offer: string, ephemeral_key: string;
  try {
    ({ sdp_offer, ephemeral_key } = await req.json() as { sdp_offer: string; ephemeral_key: string });
  } catch {
    return NextResponse.json({ error: "invalid_json" }, { status: 400 });
  }
  if (!sdp_offer?.trim())    return NextResponse.json({ error: "missing_sdp_offer" }, { status: 400 });
  if (!ephemeral_key?.trim()) return NextResponse.json({ error: "missing_ephemeral_key" }, { status: 400 });
