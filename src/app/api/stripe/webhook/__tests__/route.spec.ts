...
vi.mock("@/shared/subscriptions/stripeWebhookLedger", () => ({
  ...
  finishStripeWebhookEvent: (...args: unknown[]) => mocks.finishWebhook(...args),
}));
  ...
  createServiceRoleClient: () => ({
    from: (table: string) => {
      ...
      const chain = {
        ...
        eq: () => chain,
        neq: async () =>
          mocks.bookingUpdateResults.shift() ?? { error: null },
      };
      return chain;
    },
  }),
...
describe("Stripe webhook event ledger", () => {
  ...
  it("claims and completes a verified event before acknowledging it", async () => {
    ...
    expect(mocks.claimWebhook).toHaveBeenCalledWith(
      expect.objectContaining({ eventId: EVENT_ID, eventType: "customer.created" }),
    );
  ...
  it("fences concurrent delivery without acknowledging the in-flight copy", async () => {
    ...
    await vi.waitFor(() => expect(mocks.finishWebhook).toHaveBeenCalledTimes(1));
  ...
  it("fails closed when the persistent event claim is unavailable", async () => {
    mocks.claimWebhook.mockRejectedValueOnce(new Error("simulated database outage"));
