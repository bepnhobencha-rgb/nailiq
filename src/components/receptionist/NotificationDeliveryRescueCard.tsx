...
export function NotificationDeliveryRescueCard({
  ...
  if (!summary.available) {
    issues.push(
      language === "vi"
        ? "Chưa đọc được trạng thái giao thông báo."
    ...
    if (!summary.emailOutboundEnabled) {
      issues.push(language === "vi" ? "Email đang tắt." : "Email is turned off.");
    }
    ...
    if (summary.waitlistAttentionCount > 0) {
      issues.push(
        language === "vi"
          ? `${summary.waitlistAttentionCount} thông báo Waitlist cần cứu.`
          : `${summary.waitlistAttentionCount} Waitlist notifications need rescue.`,
      );
    }
  }
