...
export function PromotionsEditor({ slug, sectionId, content, onContentUpdate }: EditorProps) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div>
        <label className={labelClass}>Nội dung khuyến mãi</label>
        <textarea
          className={textareaClass}
          rows={4}
          value={typeof content.body === "string" ? content.body : ""}
          placeholder="Giảm 20% cho lần đặt lịch đầu tiên..."
      ...
      <div>
        ...
        <input
          type="date"
          className={inputClass}
          value={typeof content.expires_at === "string" ? content.expires_at.substring(0, 10) : ""}
          onChange={e => handleChange({ expires_at: e.target.value || null })}
        />
      </div>
      <div>
        <label className={labelClass}>Phong cách nền</label>
        <select
          className={selectClass}
          value={typeof content.bg_style === "string" ? content.bg_style : "brand"}
          onChange={e => handleChange({ bg_style: e.target.value })}
        >
          <option value="brand">Màu thương hiệu</option>
