...
describe("try-on image pipeline", () => {
  ...
  it.each([
    ...
    ["image/png", "png"],
    ["image/webp", "webp"],
  ] as const)("accepts a valid %s public upload and normalizes it", async (mimeType, format) => {
    const source = await sharp({
  ...
  it("rejects malformed bytes even when the browser declares an allowed MIME", async () => {
    const malformed = Buffer.from([0xff, 0xd8, 0xff, 0x00, 0x01, 0x02]);

    const result = await preparePublicTryOnUpload(malformed, "image/jpeg");

    expect(result).toEqual({ ok: false, error: "invalid_image" });
  });

  it("rejects a valid image disguised behind a different declared MIME", async () => {
