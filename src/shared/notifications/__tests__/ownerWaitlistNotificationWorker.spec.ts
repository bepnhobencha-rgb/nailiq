...
describe("owner waitlist notification outbox worker", () => {
  it("uses the durable outbox id as the provider idempotency identity", async () => {
    ...
    await expect(runOwnerWaitlistNotificationWorker(10, {
      client: h.client,
      send,
  ...
  it("keeps occurrence creation atomic, leased and browser-inaccessible", () => {
    const migration = readFileSync(resolve(
      process.cwd(),
      "supabase/migrations/20260830192628_add_waitlist_owner_notification_outbox.sql",
    ), "utf8");
    expect(migration).toContain("AFTER INSERT ON public.booking_waitlist_entries");
    expect(migration).toContain("ON CONFLICT (waitlist_entry_id) DO NOTHING");
