...
export function shouldUseGuidedFocusMode(
  ...
  return (
    stage === "incomplete" || (stage === "complete" && activeSegment === null)
  );
}
...
export function deriveGuidedSetupProgress(
  ...
  const definitions: Array<
    Omit<
      GuidedSetupStep,
      "done" | "selected" | "skipped" | "detailEn" | "detailVi"
    > & {
      checkIds: string[];
      completeEn: string;
      completeVi: string;
  ...
  > = [
    ...
    },
    {
      ...
      completeEn: "Communication fallback and default language are complete.",
      completeVi: "Kênh dự phòng và ngôn ngữ mặc định đã hoàn tất.",
    ...
    },
    {
      ...
      titleEn: "Safe booking preview",
      titleVi: "Xem trước booking an toàn",
      reasonEn:
        "A real human must confirm the customer-visible service, eligible staff, date, and current time availability before launch.",
      reasonVi:
        "Người thật phải xác nhận dịch vụ, nhân viên phù hợp, ngày và lịch trống hiện tại mà khách sẽ thấy trước khi ra mắt.",
      validationEn:
        "Passes only when the public booking gate is valid and an authorized human records a server-rechecked, side-effect-free availability preview.",
      validationVi:
        "Chỉ đạt khi điều kiện đặt lịch công khai hợp lệ và người có quyền ghi nhận preview lịch trống được server kiểm tra lại, không tạo side effect.",
      completeEn:
        "The public booking gate and authorized availability preview both pass.",
      completeVi:
        "Điều kiện đặt lịch công khai và preview lịch trống được phê duyệt đều đạt.",
      href: `${setupBase}/preview`,
    },
    {
      id: "go-live",
      required: true,
      checkIds: ["hours-confirmation", "otp-policy", "owner-approval"],
  ...
  const steps: GuidedSetupStep[] = definitions.map(
    ({ checkIds, completeEn, completeVi, ...definition }) => {
      ...
      const skipped = checkIds.some(
        (id) => findCheck(readiness, id)?.skipped === true,
      );
      return {
        ...definition,
  ...
  const complete =
    requiredSteps.every((step) => step.done) && readiness.approvedForGoLive;
  const nextStep = complete
    ? null
    : (steps.find(
        (step) =>
          (step.required && !step.done) ||
          (!step.required && !step.selected && !step.skipped),
      ) ?? null);
