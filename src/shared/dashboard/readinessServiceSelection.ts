...
): Array<Pick<ReadinessServiceCandidate, "id" | "priceCents" | "durationMinutes">> {
  return services.flatMap(
    ({ id, priceCents, durationMinutes, isAddon }) =>
      guidedSetupEnabled && isAddon
        ? []
        : [{ id, priceCents, durationMinutes }],
  );
}
