...
async function sendToEachRecipientDetailed(
  ...
  meta: {
    ...
    event: string;
    eventOccurrenceKey?: string | null;
    /** Stable, PII-free outbox identity for provider idempotency. */
  ...
  const results: RecipientDispatchOutcome[] = await Promise.all(
    recipients.map(async (rawRecipient) => {
      ...
      try {
        const waitlistIdempotencyKey = meta.waitlistDeliveryId
            ...
            ? await resend.emails.send(
                {
                  ...
                  tags: [
                    ...
                    {
                      name: "nailiq_delivery",
                      value: meta.waitlistDeliveryId!,
                    },
                  ],
                },
 ...
 * slot was full). Delivery is invoked only by the leased Waitlist outbox
 * worker. A stable delivery id becomes the provider idempotency identity, so a
 * timeout/retry can never create duplicate accepted emails.
 */
