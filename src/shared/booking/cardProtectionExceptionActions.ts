...
export type CardProtectionException = {
  ...
  lastAttemptAt: string | null; reviewedAt: string | null;
};
export async function loadCardProtectionExceptions(slug: string): Promise<{ ok: boolean; items: CardProtectionException[]; hasMore?: boolean }> {
  ...
  const { data, error } = await db.from("bookings" as never)
    .select("id,client_name,start_time_utc,card_protection_status,services!bookings_service_id_fkey(name)")
  ...
  const bookings = data as { id: string; client_name: string | null; start_time_utc: string; card_protection_status: unknown; services: { name: string } | null }[];
  ...
  const { data: operations, error: opError } = await db.from("booking_card_save_operations" as never)
    .select("booking_id,first_failure_stage,first_failure_code,updated_at,reviewed_at")
  ...
  const rows = (operations ?? []) as { booking_id: string; first_failure_stage: string | null; first_failure_code: string | null; updated_at: string; reviewed_at: string | null }[];
  return { ok: true, hasMore: bookings.length > 100, items: bookings.slice(0,100).flatMap((booking) => {
    ...
    return [{ bookingId: booking.id, clientLabel: initials ? `${initials}.` : "Guest",
      ...
      failureStage: operation?.first_failure_stage ?? null, failureCode: operation?.first_failure_code ?? null,
      lastAttemptAt: operation?.updated_at ?? null, reviewedAt: operation?.reviewed_at ?? null }];
  }) };
}
...
export async function actOnCardProtectionException(slug: string, bookingId: string,
  ...
  if (action === "reconcile") {
    ...
    const { data, error: reviewError } = await db.rpc("mark_booking_card_exception_reviewed" as never,
      { p_operation_id: operationId, p_salon_id: ctx.salon.id, p_actor_id: ctx.userId } as never);
    ok = !reviewError && (data as { ok?: boolean } | null)?.ok === true;
  }
