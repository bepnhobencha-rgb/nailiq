...
export type GroupCancellationFeeReviewQueueItem = {
  reviewId: string;
  groupId: string;
  ...
  cardBrand: string;
  cardLast4: string;
  state: "pending_review" | "approved_charge" | "waived" | "not_applicable";
...
function stableApprovalRequestId(reviewId: string, action: "charge" | "waive"): string {
  const bytes = createHash("sha256")
    .update(`nailiq:group-cancellation-fee-approval:v1:${reviewId}:${action}`, "utf8")
    .digest()
    .subarray(0, 16);
...
export async function loadGroupCancellationFeeReviewQueue(
  slug: string,
): Promise<GroupCancellationFeeReviewQueueItem[]> {
  ...
  return rows.filter((row) => row.state !== "not_applicable").map((row) => {
    const booking = byId.get(String(row.organizer_booking_id));
    const service = booking?.services as { name?: unknown } | Array<{ name?: unknown }> | null;
    ...
    return {
      ...
      state: String(row.state) as GroupCancellationFeeReviewQueueItem["state"],
...
export async function decideGroupCancellationFeeReview(
  slug: string,
  input: {
    salonId: string;
    reviewId: string;
...
): Promise<{ ok: true; state: string; paymentStatus: string } | { ok: false; error: string }> {
  const ctx = await getDashboardWriteClient(slug);
  ...
  const { data, error } = await createServiceRoleClient().rpc(
    "decide_group_cancellation_fee_review" as never,
    {
      p_review_id: input.reviewId,
      p_salon_id: ctx.salon.id,
      p_actor_user_id: ctx.userId,
      p_actor_role: String(ctx.role),
      p_approval_request_id: stableApprovalRequestId(input.reviewId, input.action),
