...
async function loadSmsTemplateDispatchPolicy(
  salonId: string,
  notificationType: string | undefined,
): Promise<
  | { allowed: true }
  | { allowed: false; disposition: "suppressed" | "unknown"; reason: string }
> {
  if (!notificationType || !isSmsTemplateKey(notificationType)) {
    return { allowed: true };
  }
  if (isRequiredSmsTemplate(notificationType)) return { allowed: true };
  try {
    const { createServiceRoleClient } = await import(
      "@/shared/lib/supabase/serviceRole"
    );
    ...
    if (error) {
      return {
        allowed: false,
        disposition: "unknown",
        reason: "sms_template_policy_unavailable",
      };
    }
    const settings = (data as { settings?: unknown } | null)?.settings;
    return isSmsTemplateEnabled(notificationType, settings)
      ? { allowed: true }
      : {
          allowed: false,
          disposition: "suppressed",
...
export async function sendSmsReminder(
  ...
  const templatePolicy = await loadSmsTemplateDispatchPolicy(
    opts.salonId,
    opts.notificationType,
  );
  if (!templatePolicy.allowed) {
    return templatePolicy.disposition === "suppressed"
      ? suppressBeforeProvider(templatePolicy.reason)
      : rejectBeforeProvider(templatePolicy.reason);
  }
