 ...
 *
 * Callers are responsible for their own access gating — this module performs
 * the salon lookup per tool but does NOT re-check voice_ai_enabled.
 *
 * `baseUrl` is used for Party Link generation on group bookings.
 */
export async function executeVoiceTool(
  ...
  if (toolName === "confirm_group_booking") {
    return handleConfirmGroupBooking(supabase, salonSlug, toolArgs, sessionId, baseUrl);
  }
...
// ─── lookup_customer ─────────────────────────────────────────────────────────
// The "memory" tool: given a phone number, return everything the salon already
// knows about this customer so the receptionist can greet them by name, suggest
// their usual service/staff, and avoid allergens. Reads only — never mutates.
//
// Privacy rules enforced here (NOT left to the model):
//   • consent_ai_personalization === false → name only, no history/preferences.
//   • is_vip / total spend / internal owner notes are NEVER returned.
async function handleLookupCustomer(
  ...
  // Same last-9-digit suffix match as find_booking (handles +84 / +1 / spaces).
  const digits = customerPhone.replace(/\D/g, "");
  const last9  = digits.slice(-9);
  if (last9.length < 7) {
    return NextResponse.json({ known: false, hint: "Phone number too short to look up. Continue the normal flow." });
  }
  ...
  if (sessionId) {
    try {
      await supabase
        .from("voice_ai_sessions")
        .update({ client_phone: digits, client_name: profile.name ?? null })
        .eq("id", sessionId);
    } catch { /* best-effort */ }
  }

  const [prefsRes, summaryRes, patternRes] = await Promise.all([
    supabase
    ...
    supabase
      ...
      .select("summary_text, visit_count")
      .eq("salon_id", salon.id)
      .eq("client_profile_id", profile.id)
      .maybeSingle(),
    supabase
  ...
  const summary = summaryRes.data as { summary_text: string | null; visit_count: number | null } | null;
  const visitCount = summary?.visit_count ?? profile.visit_count ?? 0;
  ...
  return NextResponse.json({
    ...
    hint:
      ...
      "never recite the list. NEVER mention spend, internal notes, or that you 'looked them up'.",
  });
}
...
// salon's configured channel (email / SMS / both). Alert failure never fails
// the tool: the logged row is the source of truth.
