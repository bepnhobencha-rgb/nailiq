...
async function buildResolverContext(
  ...
  const svcByName = new Map<string, { id: string; price: number }>(
    (svcRows ?? []).map((s): [string, { id: string; price: number }] => [
      normKey(s.name),
      { id: s.id as string, price: (s.price_cents as number) ?? 0 },
    ]),
  );
  const { data: staffRows } = await db
    .from("staff")
    .select("id,name,wix_resource_id")
    .eq("salon_id", salonId);
  const staffByResource = new Map<string, string>(
    (staffRows ?? [])
      .filter((s) => s.wix_resource_id)
      .map((s): [string, string] => [
        s.wix_resource_id as string,
        s.id as string,
      ]),
  );
  const staffByName = new Map<string, string>(
    (staffRows ?? []).map((s): [string, string] => [
      normKey(s.name),
      s.id as string,
    ]),
  );
  const { data: salonRow } = await db
    .from("salons")
    .select("verification_risk_threshold_otp")
    .eq("id", salonId)
    .maybeSingle();
  const otpThreshold = Number(
    (salonRow?.verification_risk_threshold_otp as number) ?? 30,
  );
  return {
    db,
    salonId,
  ...
  if (resourceId) {
    const { data } = await ctx.db
      ...
      .insert({
        ...
        job_role: "nail_tech",
        status: "active",
        wix_resource_id: resourceId,
      })
      .select("id")
      .single();
    const newId = data?.id as string | undefined;
    if (newId) {
      ctx.staffByResource.set(resourceId, newId);
      ctx.staffByName.set(normKey(label), newId);
    }
    return newId ?? null;
  }
...
async function evaluateClient(
  ...
  phone: string,
  subtotalCents: number,
): Promise<{ risk: number; approve: boolean }> {
...
async function _processOne(
  ctx: ResolverContext,
  b: WixBooking,
): Promise<BookingEventResult & { autoConfirmed: boolean }> {
  const now = Date.now();
  ...
  const end =
    b.endDate ??
    b.bookedEntity?.slot?.endDate ??
    new Date(new Date(start).getTime() + 45 * 60000).toISOString();
  const ph = canonPhone(b.contactDetails?.phone);
  const note =
    (b.additionalFields ?? []).find((f) => f.label === "Add Your Message")
      ?.value || null;
  ...
  // Resolve the tenant-bound identity before any mutable service/staff/client
  // prerequisite. A terminal provider replay must be handled (or rejected)
  // without creating unrelated local records first.
  let found = (
    await ctx.db
      .from("bookings")
      .select("id, status")
      .eq("salon_id", ctx.salonId)
      .eq("wix_booking_id", b.id)
      .maybeSingle()
  ).data;
  if (!found) {
    let q = ctx.db
  ...
  // Terminal NailIQ history is immutable. Wix may replay a cancellation with
  // stale mutable fields; never rewrite a cancelled/no-show tombstone. A
  // cancellation for an unknown booking is also not imported as a synthetic
  // terminal row: the active create event (or the next full sync) must establish
  // identity first.
  if (
    (existingId && ["cancelled", "no_show"].includes(String(found?.status))) ||
    (!existingId && status === "cancelled")
  ) {
    return { action: "skipped", autoConfirmed: false };
  }

  // Wix cancellation is a terminal fact. Route it through the same
  // tenant-scoped, audited system cancellation capability used by trusted
  // customer channels; never let this sync bypass the terminal boundary with
  // a direct UPDATE. Unknown terminal rows remain fail-closed above.
  if (existingId && status === "cancelled") {
    const { data: cancelData, error: cancelError } = await ctx.db.rpc(
    ...
    if (cancelError || cancellation?.ok !== true) {
      return { action: "skipped", autoConfirmed: false };
    }
    return {
      action: "updated",
  ...
  const staffId = await resolveStaff(
    ctx,
    b.bookedEntity?.slot?.resource?.id,
    b.bookedEntity?.slot?.resource?.name ?? "",
  );
  ...
  if (ph) {
    await ctx.db.from("client_profiles").upsert(
      {
        phone: ph,
        name: clientName === "Guest" ? null : clientName,
  ...
  const fields: Record<string, unknown> = {
    ...
    client_phone: ph,
    client_email: b.contactDetails?.email ?? null,
    service_id: svc.id,
    staff_id: staffId,
    ...
    staff_requested_by_client: noteRequestsStaff(note),
  ...
  if (existingId) {
    ...
    const insertRow = {
      ...fields,
      no_show_risk_score: computedRisk ?? 8,
    ...
    if (error && staffId)
      ({ data, error } = await ctx.db
        .from("bookings")
        .insert({ ...insertRow, staff_id: null })
        .select("id")
        .single());
    if (error) {
      action = "skipped";
    } else {
      bookingId = data?.id as string | undefined;
      action = "inserted";
    }
  }
...
export async function runForwardSync(
  ...
  for (const b of bookings) {
    ...
    if (result.autoConfirmed && result.bookingId) {
      autoApproved++;
      void pushWixConfirm(salonId, result.bookingId);
    }
