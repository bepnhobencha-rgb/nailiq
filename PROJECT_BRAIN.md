...
- **Design foundation** — `src/shared/theme/tokens.ts` (TypeScript design tokens, including `breakpoints` and `layout`) and `src/app/globals.css` (Tailwind v4 `nq-*` theme, layout CSS vars, `pb-safe` / `pb-safe-cta`, liquid-glass `glass` / `glow-gold` / `text-shimmer` / `fade-in`, `shimmer` + `fadeIn` keyframes, plus `animate-nq-orb`, `animate-nq-content-pulse`, `animate-nq-phone-tick` / `nq-phone-tick`; `prefers-reduced-motion` disables heavy motion). Reusable UI in `src/components/ui/` (`Button`, `Card`, `Input`, `Badge`, `Section`, `PhoneFrame` + `src/components/ui/phone/*` helpers) and layout in `src/components/layout/` (`ResponsiveShell`, `MobileStack`, `DesktopSplit`) plus `src/shared/lib/cn.ts` and `useDocumentPhoneScene` for the marketing phone. **Marketing:** `MarketingPhoneAside` composes the hero **PhoneFrame** (no file-upload CTA) on home (`/`). **Alternate conversion page** — `/aggressive`: single-column, urgent copy (loss framing + money counter + lists + `AggressivePhoneDemo` with **hero** scene only); `getAggressiveMessages` in `src/shared/i18n/user/aggressive.ts`. **SEO:** `src/shared/seo/` (`site.ts`, `jsonLd.ts`), root `metadata`, `robots.ts`, `sitemap.ts`, `public/ai.txt`, `public/llms.txt`. **i18n:** `src/shared/i18n/user/` (EN/VI) + `useUserLanguage`; booking strings only in `src/shared/i18n/booking/en.ts`. **Phone mock copy:** `phoneTimeSlots` + `serviceStrip` + `phoneBookingSceneTitle` + `phoneBookingStep*` / `phoneBookingContinue` / `phoneBookingDateLine` (booking demo) + scroll-scene and AI-demo strings in user `en`/`vi` (not tenant data). Aggressive page copy lives in `aggressive.ts` (incl. **`phoneAppBar`**, **duration/price** for cards, **four** time slots, **bottom-sheet** CTA lines). The long-form **`/aggressive` page** `PhoneDemo` is a custom in-frame mock (cards + horizontal time pills + light bottom sheet, **`niq-pop`** + **`niq-check`** in `globals.css`); `AggressivePhoneDemo` remains the **hero-only** `PhoneFrame` on the short landing.
- **Public booking** — `src/app/[shop]/page.tsx` is **English only**; client **`BookingFlow`**: service → time → confirm → success; catalog + **`submitPublicBooking`** stub in `src/shared/booking/`. `BookingDocumentEn` sets `html` `lang` to `en`. Not mixed with user locale.
