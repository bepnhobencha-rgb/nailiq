...
- **Marketing layer (PROMPT #017) — do not break UI:** The landing **keeps the same layout tokens** (grids, gaps, CTA and input size). It **adds** only: an **urgency** one-liner under the CTA (subtle `nq-urgency-breathe` fade, muted type); **live proof** (rotating muted lines under the input, 2.6s interval, `aria-hidden` for SR); **phone mock** polish inside `PhoneFrame` (active-scale interaction, service **highlight** pill + `fade-in` on rotate, 2.4s strip interval). The mental model remains **urgency + proof + product-first**—elegant, not spam. **Default expectation:** new marketing work should preserve **all three** (light-touch urgency, social-style proof, dominant product) without a layout redesign.
...
- **Design foundation** — `src/shared/theme/tokens.ts` (TypeScript design tokens, including `breakpoints` and `layout`) and `src/app/globals.css` (Tailwind v4 `nq-*` theme, layout CSS vars, `pb-safe` / `pb-safe-cta`, liquid-glass `glass` / `glow-gold` / `text-shimmer` / `fade-in`, `shimmer` + `fadeIn` keyframes, plus `animate-nq-orb`, `animate-nq-content-pulse`, `animate-nq-phone-tick` / `nq-phone-tick`; `prefers-reduced-motion` disables heavy motion). Reusable UI in `src/components/ui/` (`Button`, `Card`, `Input`, `Badge`, `Section`, `PhoneFrame` + `src/components/ui/phone/*` helpers) and layout in `src/components/layout/` (`ResponsiveShell`, `MobileStack`, `DesktopSplit`) plus `src/shared/lib/cn.ts` and `useDocumentPhoneScene` for the marketing phone. **Marketing:** `MarketingPhoneAside` composes the hero **PhoneFrame** (no file-upload CTA). Home (`/`) is a high-emotion, conversion-leaning **foundation preview** (headline, CTA, phone helper, social proof, and **SEO-visible** intro + benefits; not final marketing shipped product). **SEO:** `src/shared/seo/` (`site.ts`, `jsonLd.ts`), root `metadata`, `robots.ts`, `sitemap.ts`, `public/ai.txt`, `public/llms.txt`. **i18n:** `src/shared/i18n/user/` (EN/VI) + `useUserLanguage`; booking strings only in `src/shared/i18n/booking/en.ts`. **Phone mock copy:** `phoneTimeSlots` + `serviceStrip` + `phoneBookingSceneTitle` + `phoneBookingStep*` / `phoneBookingContinue` / `phoneBookingDateLine` (booking demo) + scroll-scene and AI-demo strings in user `en`/`vi` (not tenant data).
