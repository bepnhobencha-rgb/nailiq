...
- **Public booking** — `src/app/[slug]/page.tsx`, `BookingFlow` + `submitPublicBooking` in `src/shared/booking/`; **`create_public_booking` RPC v2.3** (migration `20260502130000_public_booking_conflict_check.sql`) uses desk-aligned overlap (`pending`/`confirmed`/`in_progress`/`completed` only) + transactional advisory lock; conflict → JSON `slot_conflict` or client `BookingConflictError` via `submitPublicBooking`.
...
- **Operational screen** at `/dashboard/[slug]/center` (separate from owner analytics `/dashboard/[slug]`). **Empty setup (Task 5):** When **`services.length === 0` OR `staff.length === 0`**, a **warning banner** (`nq-warning`) + **smart CTA**: **`setup/services`** if no services yet, else **`setup/staff`** (services exist but no staff); **`WalkinAddForm`** receives **`disabled`** (grayed controls, no submit). Copy: **`receptionist.setupIncompleteBanner`** EN/VI. E2e: **`empty-salon.spec.ts`** (slugs `e2e-empty-salon-center`, `e2e-empty-salon-no-staff`). **E2e:** `e2e/receptionist-center/*.spec.ts` + `helpers.ts` — **`npm run test:e2e`** (**63 passed**, **1 skipped** on Playwright **mobile** for ghost-hover-only coverage); linked DB + `SUPABASE_SERVICE_ROLE_KEY`. Adds **`public-booking-race.spec.ts`** — walk-in-held slot vs **`create_public_booking`** (`slot_conflict`) and appointment vs desk assign (grid conflict toast), each under **chromium** + **mobile**. Adds **`service-delete.spec.ts`** / **`staff-delete.spec.ts`**. **Staff delete data rule:** application-level detach of **`cancelled`/`completed`** bookings’ **`staff_id`** before delete — see **`decisions-log.md`** (2026-05-02 staff delete / terminal bookings). **Step 2 done:** `BookingStatus` / `BookingSource`, `OWNER_TODAY_LIST_STATUSES` vs `ACTIVE_GRID_STATUSES`, dashboard booking map + owner loader filter + today split (see CHANGELOG). **Step 4:** `receptionistActions.ts` queue/assign/undo/start mutations + **`cancelWaitingWalkin`**, **`cancelDeskBooking`** (grid cancel → `cancelled`). **Step 5:** `loadReceptionistCenterData.ts` (single payload for center UI; join flattening like `dashboardBookingMap`). **Step 6a:** Presentational receptionist leaves in `src/components/receptionist/` — `BookingBlock`, `NowLine`, `GhostBlock`, `StatusPill`, `UndoToast` (`"use client"`, tokens/`cn()`, no fetching). **Step 6b:** `DateSwitcher`, `WalkinAddForm`, `WalkinQueueSidebar`, `StaffTimelineGrid` — compose 6a + `checkBookingConflict` / `salonTime` / `queueUrgency`; no fetch, no server actions in-container (containers used by Step 6c-1 caller). Warn token `nq-warning` in `tokens.ts` + `globals.css` for busy pill. **Step 6c-1:** `ReceptionistCenter.tsx`, `BookingDetailDrawer.tsx` (**Start service** / **Mark complete** / **Cancel** by status via `updateBookingStatus` + `cancelDeskBooking`), `loadReceptionistCenterDataAction.ts`, user **`receptionist`** i18n — loaders + mutations wired; **`updateBookingStatus`** supports `pending`/`confirmed` → `in_progress` + `started_at` (atomic guard). **Step 6c-2:** RSC **`center/page.tsx`**, Receptionist **`nowIso`** 60s tick, undo toast **5s** countdown (1s tick, `showCountdown`), Supabase **`bookings`** realtime (session) vs **8s** poll (no JWT). Production callers load with `loadReceptionistCenterData(slug, dateYmd)` two-arg only (`deps` is test injection only). Regenerate `database.types.ts` after linked schema changes (`npx supabase gen types typescript --project-id fshmobzyjhmtvndobwsy > src/lib/database.types.ts`).
