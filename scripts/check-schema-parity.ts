 ...
 * The 20260831013000/20260831021500 late-cancellation and approved group-fee
 * dispatch migrations add two private service-role-only late-fee review and
 ...
 * TurnIQ Live Readiness adds two private rollout control/audit tables, 18
 * columns, two restrictive policies, one service-only transition function,
 * one append-only guard function/trigger, and five primary/unique/history
 * indexes. It creates no rows.
 * TurnIQ Controlled SHADOW Activation adds two private allowlist/receipt
 * tables, 21 columns, two restrictive browser-deny policies, one
 ...
 * TurnIQ M3I staff PIN check-in adds three private credential/receipt tables,
 * 28 columns, three private/service-only functions, five tenant/immutability
...
const CRITICAL_FUNCTIONS = [
  ...
  "booking_late_cancellation_snapshot",
  "protect_late_cancellation_lock_policy",
  "prevent_late_cancellation_fee_receipt_mutation",
  "capture_late_cancellation_fee_review",
  "decide_late_cancellation_fee_review",
...
function main() {
  ...
  for (const table of NO_SHOW_FEE_SERVICE_ONLY_TABLES) {
    ...
    const ok =
      browserReachable === 0 && serviceReachable === 1 && forcedRls === 1;
    if (!ok) failed = true;
    console.log(
      `  ${ok ? "✓" : "✗"} ${table}` +
        (ok
