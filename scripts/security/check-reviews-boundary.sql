...
DO $check$
DECLARE
  v_oid oid := to_regclass('public.reviews');
  v_policy_count integer;
BEGIN
  IF v_oid IS NULL THEN
    RAISE EXCEPTION 'reviews is missing';
  END IF;

  IF NOT (SELECT relrowsecurity FROM pg_class WHERE oid = v_oid) THEN
    RAISE EXCEPTION 'RLS is disabled on reviews';
  END IF;
  ...
  IF has_table_privilege(
    ...
    RAISE EXCEPTION 'direct API grants remain on reviews';
  END IF;
  ...
  IF NOT has_table_privilege(
    ...
    RAISE EXCEPTION 'service_role access is incomplete on reviews';
  END IF;
  ...
  IF v_policy_count <> 1 OR NOT EXISTS (
    ...
    RAISE EXCEPTION 'explicit reviews deny policy mismatch';
  END IF;
END
$check$;
