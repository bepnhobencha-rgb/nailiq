...
DO $check$
DECLARE
  v_oid oid := to_regclass('public.booking_addons');
  v_policy_count integer;
BEGIN
  IF v_oid IS NULL THEN
    RAISE EXCEPTION 'booking_addons is missing';
  END IF;

  IF NOT (SELECT relrowsecurity FROM pg_class WHERE oid = v_oid) THEN
    RAISE EXCEPTION 'RLS is disabled on booking_addons';
  END IF;
  ...
  IF has_table_privilege(
    ...
    RAISE EXCEPTION 'direct API grants remain on booking_addons';
  END IF;
  ...
  IF NOT has_table_privilege(
    ...
    RAISE EXCEPTION 'service_role access is incomplete on booking_addons';
  END IF;
  ...
  IF v_policy_count <> 1 OR NOT EXISTS (
    ...
    RAISE EXCEPTION 'explicit booking_addons deny policy mismatch';
  END IF;
  ...
  IF NOT has_function_privilege(
    ...
    RAISE EXCEPTION 'add_booking_addons role boundary mismatch';
  END IF;
END
$check$;
