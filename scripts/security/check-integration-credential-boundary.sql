...
BEGIN
  ...
  LOOP
    ...
    IF v_oid IS NULL THEN
      RAISE EXCEPTION 'integration table is missing: %',
        v_target.table_name;
    END IF;
    ...
    IF has_table_privilege('anon', v_oid, 'SELECT,INSERT,UPDATE,DELETE,TRUNCATE,REFERENCES,TRIGGER')
       ...
       OR has_any_column_privilege('anon', v_oid, 'SELECT,INSERT,UPDATE,REFERENCES')
       OR has_any_column_privilege('authenticated', v_oid, 'SELECT,INSERT,UPDATE,REFERENCES') THEN
      RAISE EXCEPTION 'direct API grants remain on %', v_target.table_name;
    END IF;
    ...
    IF NOT has_table_privilege(
      ...
      RAISE EXCEPTION 'service_role integration access is incomplete on %',
        v_target.table_name;
    END IF;
