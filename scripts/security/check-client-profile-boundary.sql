...
DO $check$
DECLARE
  v_oid oid := to_regclass('public.client_profiles');
  v_policy_count integer;
BEGIN
  IF v_oid IS NULL THEN
    RAISE EXCEPTION 'client_profiles is missing';
  END IF;

  IF NOT (SELECT relrowsecurity FROM pg_class WHERE oid = v_oid) THEN
    RAISE EXCEPTION 'RLS is disabled on client_profiles';
  END IF;
  ...
  IF has_table_privilege(
    ...
    RAISE EXCEPTION 'direct API grants remain on client_profiles';
  END IF;
  ...
  IF NOT has_table_privilege(
    ...
    RAISE EXCEPTION 'service_role access is incomplete on client_profiles';
  END IF;
  ...
  IF v_policy_count <> 1 OR NOT EXISTS (
    ...
    RAISE EXCEPTION 'explicit client_profiles deny policy mismatch';
  END IF;
END
$check$;
