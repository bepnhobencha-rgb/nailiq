...
try{
 ...
 const requestJson=`'{"source_id":"__full_scan__","secondary_id":"__first_page__"}'::jsonb`;
 const fp=await run(`select set_config('request.jwt.claim.role','service_role',true); select public.resolve_square_feature_operation_material('${salon}','inventory_catalog_variation_load',${requestJson})->>'material_fingerprint'`); const hash=fp.split("\n").at(-1);
 const claim=`select set_config('request.jwt.claim.role','service_role',true); select public.claim_square_feature_operation('${salon}','${request}','inventory_catalog_variation_load',${requestJson},'${hash}')::text`;
 ...
 const blocked=JSON.parse((await run(`select set_config('request.jwt.claim.role','service_role',true); select public.claim_square_feature_operation('${salon}','51250000-0000-4000-8000-000000000003','inventory_adjustment','{"source_id":"variation_race","quantity":"1.5","from_state":"IN_STOCK","to_state":"SOLD"}'::jsonb,repeat('a',64))::text`)).split("\n").at(-1));
 assert.equal(blocked.code,"specialized_inventory_claim_required");
 assert.equal(await run(`select count(*) from public.square_feature_operations where salon_id='${salon}' and request_id='51250000-0000-4000-8000-000000000003'`),"0");
