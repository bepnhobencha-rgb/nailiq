...
try{
 ...
 insert into public.booking_no_show_decisions(id,salon_id,booking_id,state,original_status,requested_by_user_id,requested_by_role,assist_reason_code,requested_at,commit_after,committed_at) values
 ('${noDecision}','${salon}','${noshow}','committed','confirmed','${noApprover}','owner','desk_observation',clock_timestamp()-interval '2 minutes',clock_timestamp()-interval '1 minute',clock_timestamp());
 insert into public.booking_no_show_fee_reviews(id,salon_id,booking_id,no_show_decision_id,state,amount_cents,currency,group_scope,card_brand,card_last4,consent_at,consent_policy_version,consent_snapshot_hash,ai_recommendation,ai_reason_codes,requested_by_user_id,requested_by_role,decided_by_user_id,decided_by_role,decided_at,approval_request_id,payment_status) values
 ('${noReview}','${salon}','${noshow}','${noDecision}','approved_charge',1500,'CAD','booking_member','visa','4242',clock_timestamp(),'nsp_'||repeat('a',64),repeat('b',64),'charge','["attendance_committed"]'::jsonb,'${noApprover}','owner','${noApprover}','owner',clock_timestamp(),'${noApprovalRequest}','dispatching');
 insert into public.booking_no_show_fee_approval_receipts(salon_id,booking_id,review_id,no_show_decision_id,approval_request_id,action,amount_cents,currency,group_scope,card_brand,card_last4,consent_at,consent_policy_version,consent_snapshot_hash,ai_recommendation,ai_reason_codes,approved_by_user_id,approved_by_role)
 select salon_id,booking_id,id,no_show_decision_id,approval_request_id,'charge',amount_cents,currency,group_scope,card_brand,card_last4,consent_at,consent_policy_version,consent_snapshot_hash,ai_recommendation,ai_reason_codes,decided_by_user_id,decided_by_role from public.booking_no_show_fee_reviews where id='${noReview}';
 ...
 const noClaim=noClaims.find((x)=>x.code==="claimed");
 const noCompleted=json(await sql(`begin;set local role service_role;select public.complete_booking_payment_operation('${noClaim.operation_id}','${noClaim.attempt_token}','succeeded','succeeded','pi_noshow_concurrency',null,null)::text;commit;`));
 assert.equal(noCompleted.code,"succeeded",JSON.stringify(noCompleted));
 const noRefundMat=json(await sql(`begin;set local role service_role;select public.load_booking_payment_operation_material('${salon}','${noshow}','noshow_refund',900)::text;commit;`));
 const remainingRefundClaims=(await Promise.all([301,302].map((n)=>sql(`begin;set local role service_role;select public.claim_booking_payment_operation('${salon}','${noshow}',('15160000-0000-4000-8000-'||lpad('${n}',12,'0'))::uuid,'noshow_refund',900,'${noRefundMat.material_fingerprint}')::text;commit;`)))).map(json);
 assert.equal(remainingRefundClaims.filter((x)=>x.code==="claimed").length,1,JSON.stringify(remainingRefundClaims));
 assert.equal(remainingRefundClaims.filter((x)=>["material_changed","refund_amount_exceeds_remaining"].includes(x.code)).length,1,JSON.stringify(remainingRefundClaims));
 ...
 const lateBlocks=await Promise.all(Array.from({length:10},(_,i)=>sql(`begin;set local role service_role;do $gate$ begin perform public.claim_booking_payment_operation('${salon}','${late}',('15160000-0000-4000-8000-'||lpad('${200+i}',12,'0'))::uuid,'late_cancel_charge',1500,'${lateMat.material_fingerprint}');raise exception 'late-cancel concurrency bypassed approval gate';exception when sqlstate 'NI009' then null;end $gate$;select 'blocked';commit;`)));
 assert.equal(lateBlocks.filter((x)=>x==="blocked").length,10,JSON.stringify(lateBlocks));
 assert.equal(await sql(`select count(*) from public.booking_payment_operations where salon_id='${salon}' and booking_id='${late}' and operation_kind='late_cancel_charge'`),"0");
