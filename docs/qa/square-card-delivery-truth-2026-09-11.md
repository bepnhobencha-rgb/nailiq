...
**Cập nhật gần nhất:** Phần xác minh thẻ cũ và bản chuyển tiếp tạm dừng ở mục 16 đã kiểm thử local/QA; chưa tính vào bằng chứng CI/Preview của SHA `916bdd9e`.
...
## 15. Approved branch release and hosted Preview continuation

Huy approved the explicit QA isolation, Sandbox OAuth remediation, scoped commit/push, draft PR and Preview package. Draft PR: https://github.com/bepnhobencha-rgb/nailiq/pull/1397. Merge and Production changes remain unauthorized. The default Preview environment was isolated using 23 exact-branch overrides before manual Preview creation; QA database `osdqutwunokiielbairj`, app email/SMS/call OFF and all charge dispatch OFF. Only this branch disables automatic Git deployments so a checked candidate can be deployed manually. Production metadata remains at `15fe1091fd71eda4a67704d08e5ab535967e5904` at the last read.

The exposed Sandbox OAuth authorization was revoked and recreated for the exact QA app/account. Read-only verification: old access returns 401, new access returns 200; merchant/location/CAD match, five required scopes, all webhook subscriptions disabled. Private access/refresh material stays outside Git. Provider email/SMS permission is not evidence of actual message delivery.

Initial CI failed on stale schema counts and test fixtures. Updated exact clean-schema expectations to 237 tables, 3658 columns, 539 functions, 158 triggers, 973 indexes, 218 policies and direct grants 56/78/224. Updated related inventory assertions, the router mock, the classified static exception filter and chargeable voice fixtures; six additional cases assert unprotected projections cannot return a fee. No browser grants were relaxed.

Hosted Computer Use found a recovery defect after a synthetic `.test` email was rejected by Square SearchCustomers with HTTP400/INVALID_VALUE: correcting the booking still reused the original operation/customer-claim email. This is a newly reproduced QA recovery defect, not the proven cause of the historical Production incident. Both failed operations are retained and no customer/card creation occurred in those attempts.

Forward migration `20260911104137_refresh_undispatched_card_customer_contact.sql` replaces two existing service-only functions. A fresh operation may use corrected contact only before any possible customer creation; a same-phone claim can refresh its request body under the expired-lease/row-lock fence, retaining the key/reference. Known customers, legacy ambiguity and claims with any dispatch timestamp keep frozen identity/body even after empty-read exhaustion. No historical booking or event rewrite. Roll back this follow-up by restoring the two definitions from migration `20260911040747` with CREATE OR REPLACE and unchanged service-only grants; this reintroduces the contact-recovery limitation, so pausing new card dispatch is preferable while investigating.

Validation for this batch: full unit suite 4881 passed, 30 opt-in skipped, zero failed; disposable PostgreSQL plus simulated Square integrations 21/21 including two new contact corrections and prior race/unknown-body preservation; SQL rehearsal 18 scenarios PASS locally and on hosted disposable QA in a rolled-back synthetic transaction. New contact regression was first shown failing against the old functions. Touched TypeScript lint 31 files PASS; local webpack Next build then standalone typecheck PASS. Initial CI failures are not yet a green final candidate; CI must rerun after publication.

Hosted Preview `dpl_HJWpQobdVZ1ezt2azX6RfnRZ1QpV` at application SHA `be252598b378acf38442400d65b92ee68b04a15a`: actual Square SDK save now succeeds after the QA migration; booking remains confirmed with VISA/last4, customer/card binding, consent and successful durable operation. UI says Card protection active and reload preserves it. Evidence: `preview-save-success.png`. Remaining hosted decline/retry, Owner exception and final-SHA Preview verification are in progress. This is QA/Preview evidence only; nothing has been merged, deployed to Production or Production-verified.


### Hosted browser acceptance and remaining CI fixture corrections

Computer Use on hosted Preview with real Square Sandbox SDK now verifies: successful save; successful status after reload; explicit CreateCard HTTP400/INVALID_CARD_DATA with `new_card` retryability; appointment reserved/unprotected after rejection; Owner initials-only exception, reason/time, reviewed marker surviving reload; repeated retry-link generation returning the same capability; foreign synthetic-salon navigation returning to the authorized salon; fresh blank form and unchecked consent through the Owner-generated link; successful new-card retry on the same booking. Screenshots: `preview-save-success.png`, `preview-decline.png`, `preview-owner-exceptions.png`, `preview-decline-retry-success.png` in the private evidence directory. Three synthetic bookings now have successful card receipts; no unresolved operation remains among them. Customer claims include the retained never-dispatched rejected-email claim plus three known customers; the rejected-card retry reused its customer.

The initial card ending0010 was accepted by CreateCard. Square documents that error fixture for CreatePayment; no payment call was made. The next fixture ending0002 produced the actual card-save rejection. Adjusted bounded hosted test plan from five to seven provider write requests: three customer creates and four card attempts (three successful, one rejected). Earlier local test ledger remains unchanged. No further provider writes planned. App messaging and all charge dispatch remain OFF. Notification delivery has not been verified.

The next CI rehearsal exposed older card-save fixtures with incomplete consent/binding. Updated booking-management completion/race and continuation fixtures to use full valid receipts, preserving the negative checks. Fee-operation SQL/race fixtures now model a durable historical card receipt in a temporary helper restricted to their synthetic booking names/salon IDs. The historical financial report fixture intentionally models legacy in-flight rows; both insert gates are restored immediately after fixture seeding and checked before reporting. No runtime gate is disabled by application code or migration.

All 57 downstream SQL/race/boundary/rollback commands from the affected CI segment passed on an empty local schema clone. One unrelated default-off platform flag row was seeded because schema-only cloning omits migration data; the final clean migration CI remains the authoritative folded-history gate. Full units rerun: 4881 passed, 30 opt-in skipped, zero failed. Touched JavaScript lint PASS. Application source is unchanged from the hosted browser candidate; only rehearsal fixtures and this evidence were changed in the final batch. Final SHA CI and hosted Preview confirmation remain pending publication.


### Backup rehearsal follow-up

The next folded-history CI reached the backup gate and found a session-only `pg_temp_<backend>` namespace in the source manifest. PostgreSQL retains such namespaces after temporary SQL fixtures finish; pg_dump does not restore them. The backup manifest now excludes only those numbered temporary namespaces, while keeping application schema, RLS, grants, function bodies, rows and sequence checks. The rehearsal itself creates an ephemeral probe before comparison to exercise this regression. Actual local PostgreSQL17 dump/restore PASS: 288 relations, 131 fixture rows; all four final tenant-isolation/duplicate-appointment checks also PASS. No application or migration change in this follow-up. The final hosted card receipt was reverified on Preview after an expired QA link was replaced; both expiry denial and refreshed-link success behaved correctly. Final CI rerun remains required.

## 16. Legacy-card consent recovery and schema-compatible pause bridge

Huy approved preserving existing appointments/cards, showing verification needed, and using secure links which the salon chooses to share. This follow-up implements that policy locally; it does not certify the historical Production cards or authorize Production changes. Production metadata was read again at 2026-09-11 12:50 UTC and still points to `15fe1091fd71eda4a67704d08e5ab535967e5904`. The existing `916bdd9e` PR candidate has green CI and a verified QA Preview; those results predate this follow-up.

A legacy booking with an existing card and no sending/unknown save or removal can now present fresh policy consent. The server retrieves the exact attached card through Square RetrieveCard, checks enabled state, customer/merchant/environment and full brand/last4, then compares the current booking, fee, policy and provider account with the pre-read snapshot. Completion holds short database locks only; no lock spans HTTP. Receipt freshness and capability expiry are rechecked using wall time after lock waits.

The transaction preserves card/customer IDs, booking time/status and fee. It writes one successful durable operation with `existing_card_read` provenance and fresh recovery consent; it does not fabricate a CreateCard dispatch timestamp. A revoked internal capability supplies the existing operation foreign key without revoking the customer's recovery link. Concurrent completions are idempotent. Prior consent timestamp and a fingerprint remain in the new receipt; historical operations/events are unchanged. An unresolved provider mutation must still use its operation-specific reference reconciliation, never this legacy shortcut.

Migration `20260911121118_verify_legacy_booking_cards_with_fresh_consent.sql` adds two review columns, a private append-only read-check table, six functions and the additive recovery inspection flag. Diagnostic codes/stages/status/allowlisted errors are retained without provider payloads or card secrets. Owner/Admin can mark a booking reviewed even when it has no save operation, see “Existing card — verification required”, and generate a stable secure link. Marking reviewed does not activate protection. An invalid/disabled/mismatched legacy card remains unprotected and requires salon assistance; this path never silently replaces or disables it. Stripe legacy recertification is outside this Square follow-up.

Computer Use against the local production build and actual Square Sandbox found and fixed a UI defect: after a committed database receipt lost its response, the page showed both active protection and a failure message. The UI now uses the freshly loaded durable status. A new synthetic booking reproduced the same response-loss injection and displayed only active protection. This is a newly reproduced recovery display bug, not proof of the historical Production incident's provider failure stage.

Validation at this checkpoint:

- Corrected full unit suite with a clean unit environment: **4906 PASS, 32 opt-in SKIP, zero failed tests or suites**. The earlier 4852 count omitted four suites that failed during import (`server-only`); that run was not a PASS. Those existing Square client suites now mock only the Next.js server marker, keeping the real transport logic and pause guard. All 54 previously unstarted tests pass, followed by the complete suite, typecheck and touched lint. Counts are not cumulative with prior checkpoints.
- Focused units: **58 PASS**; disposable database integrations including 20 concurrent legacy reads: **23/23 PASS**. One durable receipt, preserved binding/reservation, no provider mutation in the legacy path.
- Clean-clone SQL rehearsal: parent card truth **16 scenarios** plus new legacy verification **18 scenarios**, replay, expired/revoked capability, changed policy/card/account, malformed/disabled card, unknown-operation fence, review and private ACL: **PASS**. All fixture transactions rolled back.
- Exact candidate schema inventory: **238 tables, 3672 columns, 545 functions, 159 triggers, 975 indexes, 218 policies; direct table grants 56/78/225**. Browser reachability is unchanged. Local security advisors: zero findings.
- Main build using webpack, subsequent typecheck and touched-file lint: **PASS**. Default Turbopack could not follow this worktree's external node_modules symlink; hosted clean-checkout CI remains the Turbopack gate.
- Local Computer Use with actual existing Sandbox cards: four GET responses HTTP200, zero provider writes/charges; success + reload, simulated GET timeout + retry, database response loss before/after UI correction, Owner review without operation, repeated retry-link generation and customer link opening: **PASS**. Five new synthetic local bookings: four protected with exactly one receipt each; one remains unprotected after Owner review. App messaging remained disabled. Notification delivery was not tested.
- A combined integration run initially failed because its global cron picked up older disposable fixtures. The accepted integration run uses a separate schema clone and REST endpoint; no shared browser-fixture backlog participates. Test transport is simulated in these 23 integration cases, distinct from the four actual Sandbox GETs above.

A separate local bridge worktree at Production base `15fe1091` adds the same `NAILIQ_CARD_SAVE_DISPATCH_DISABLED=true` guards to capability saves/setup, legacy save/reuse/auto-attach, provider save adapters and the low-level Square card writer. It requires no new database objects. **14 focused tests, webpack build, typecheck and touched lint PASS.** The environment variable alone cannot pause old deployed processes; the bridge must first be deployed and old requests/processes drained. No bridge commit, PR or Production deployment is included in this checkpoint.

Rollback boundaries:

1. Before any future Production rollout, deploy the separately approved schema-compatible bridge with capture paused, verify its version/guards and drain old requests. Applying the new schema under old in-flight writers remains unsafe. Confirm the missing terminal-continuation baseline migration by actual function body, not migration-row count.
2. Apply approved forward migrations, deploy the receipt-aware candidate while paused, verify it, then obtain the separate unpause decision. No automatic cancellation or outreach. Proposed configurable grace period remains a product setting proposal, not an auto-cancel policy.
3. For a rollback of this legacy follow-up, keep its additive data/history, pause capture, and roll the application back to the receipt-aware `916bdd9e` candidate. Its protection projection understands recovery consent and the new successful receipts. Do not delete the read receipts or overwrite historical consent to make an older UI look complete.
4. A rollback across the full receipt-truth migration to the old Production schema is a separate restore/compatibility operation. Do not run the old application against a partly upgraded schema or discard a newly acknowledged provider receipt. The pause bridge rollback before schema migration requires no data rollback.

Evidence files are retained in the local task evidence directory: `legacy-cua-protected.png`, `legacy-cua-read-timeout.png`, `legacy-cua-response-loss-before-fix.png`, `legacy-cua-response-loss-fixed.png`, `legacy-cua-owner-review.png`, `legacy-cua-owner-link-customer.png`, the isolated integration/unit/SQL reports, and redacted read-only network journal. Local/QA acceptance is PASS after the unit-suite correction. Hosted Preview `5c47b030` also passed customer fresh consent, an actual Square Sandbox GET, durable protection after reload, Owner review without a prior operation, stable repeated retry-link generation and customer recovery-link opening. The reviewed-only booking remains unprotected. The follow-up to that SHA changes only test initialization and this evidence; final-SHA CI/Preview and all Production rollout/verification remain separate gates.
